...
type MyFixtures = { userCreation: User[] };

export const test = base.extend<MyFixtures>({
  userCreation: async ({}, use) => {
    const { randomString } = generator()
    const mainUser = new User(`autoqa-${randomString(5)}`,
    `autoqa-${randomString(5)}`, false)
    const adminUser = new User(`autoqa-admin-${randomString(5)}`,
    `autoqa-admin-${randomString(5)}`, true)
